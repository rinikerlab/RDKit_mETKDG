...
ForceFields::ForceField *construct3DForceField(
  ...
  const std::map<std::pair<unsigned int, unsigned int>, double> &TO_BE_NAMED_Ptr) {
    ...
    bool is1_4 = false; //FIXME should be customisable
    // double dielConst = 1.0;
    boost::uint8_t dielModel = 1;

    for (const auto& charge : TO_BE_NAMED_Ptr){
          // std::cout<< charge.first.first <<" "<< " "<<  charge.first.second << " " << " " << charge.second << std::endl << std::endl;
