...
void _setInRing14Bounds(const ROMol &mol, const Bond *bnd1, const Bond *bnd2,
  ...
  // preferCis = false;
  //std::cout << "  torsion: " << aid1 << " " << aid4 << ": " << preferCis << std::endl;
...
bool tmp_checkAmideEster14(const ROMol &mol, const Bond *bnd1, const Bond *bnd3, const Atom *atm1,
                        const Atom *atm2, const Atom *atm3, const Atom *atm4) {
  unsigned int a1Num = atm1->getAtomicNum();
  unsigned int a2Num = atm2->getAtomicNum();
  unsigned int a3Num = atm3->getAtomicNum();
  ...
  if (a2Num == 7 ) { // amide case
    //std::cout << "amidetest 1" << std::endl;
      if (mol.getAtomDegree(atm2) == 3 && mol.getAtomDegree(atm3) == 3) {
    //std::cout << "amidetest 2" << std::endl;
    ...
    //std::cout << "amidetest 3 " << res->getAtomicNum() << " " << resbnd->getBondType() << std::endl;
    ...
    //std::cout << "amidetest 4 " << res->getAtomicNum() << " " << resbnd->getBondType() << std::endl;
          if (res->getAtomicNum() != 8 || resbnd->getBondType() != Bond::DOUBLE)
            return false;
        }

    //std::cout << "amidetest 5" << std::endl;
        return true;
      }
  }
  return false;

  if (a2Num == 8 ) //TODO  ester case
    return false;
...
void _setTwoInSameRing14Bounds(const ROMol &mol, const Bond *bnd1,
  ...
  //std::cout << " in same ring: " << aid1 << " " << aid4 << ": " <<  std::endl;
  //std::cout << " I am a new random line: " << aid1 << " " << aid4 << ": " <<  std::endl;
  //std::cout << "atomic number of atom 4" << atm4->getAtomicNum();
  //std::cout << "atomic number of atom 3" << atm3->getAtomicNum();
  //std::cout << "atomic number of atom 2" << atm2->getAtomicNum();
  //std::cout << "atomic number of atom 1" << atm1->getAtomicNum();
  //std::cout << "bond order of bnd1 " << bnd1->getBondType();
  //std::cout << "bond order of bnd3 " << bnd3->getBondType();
  ...
  if ((_checkAmideEster14(bnd1, bnd3, atm1, atm2, atm3, atm4)) ||
                 (_checkAmideEster14(bnd3, bnd1, atm4, atm3, atm2, atm1))) {
                   //std::cout << "twoinsamering amidester 14" << std::endl;
// It's an amide or ester:
//
//        4    <- 4 is the O
//        |    <- That's the double bond
...
//   roam from cis to trans and hope that the force field planarizes things
//   later.
//
...
// ========================================================================
                 ...
                 (_checkAmideEster15(mol, bnd3, bnd1, atm4, atm3, atm2,
                                     atm1))) {
                   //std::cout << "twoinsamering amidester 15" << std::endl;
// it's an amide or ester.
//
//        4    <- 4 is the O
//        |    <- That's the double bond
...
// NOTE THAT WE REVERSE THE ORDER HERE:
        ...
        // aid4 << ": " << dl << "->" << du << "\n";
                   ...
                   //std::cout << "prior to amidester " << std::endl;
    ...
    accumData.transPaths[bid3 * nb * nb + bid2 * nb + bid1] = 1;
                   ...
                   //std::cout << " passed into else" << std::endl;
...
void _setChain14Bounds(const ROMol &mol, const Bond *bnd1, const Bond *bnd2,
  ...
  //std::cout << " in chain : " << aid1 << " " << aid4 << ": " <<  std::endl;

  // std::cout << "chain torsion " <<  aid1 << " " << aid4 << "\n";
...
// the user with an option to invoke this special case
      ...
      // std::cout<< "torsion indices : "  << aid1 << " " << atm2->getIdx() << " " << atm3->getIdx() << " " << aid4 << " " << std::endl;
      //
...
//   of fanciness.
//
//std::cout<< "amide14 " << std::endl;
#ifdef FORCE_TRANS_AMIDES //TODO
...
void _setMacrocycle14Bounds(const ROMol &mol, const Bond *bnd1, const Bond *bnd2,
  ...
  //std::cout << "macrocycle  torsion: " << aid1 << " " << aid4 << ": " <<  std::endl;
...
// the user with an option to invoke this special case
      ...
      if ((atm2->getAtomicNum() == 16) && (atm3->getAtomicNum() == 16)) {
        ...
        // BOOST_LOG(rdDebugLog) << "Special 9 " << aid1 << " " << aid4 <<
        // "\n";
      } else if ((tmp_checkAmideEster14(mol, bnd1, bnd3, atm1, atm2, atm3, atm4)) ||
                 (tmp_checkAmideEster14(mol, bnd3, bnd1, atm4, atm3, atm2, atm1))) {
// It's an amide or ester:
//
//        4    <- 4 is the O
//        |    <- That's the double bond
...
//   of fanciness.
//
//std::cout<< "setmacrocycle14bound amide14 " << std::endl;
// #ifdef FORCE_TRANS_AMIDES //TODO
//std::cout<< "ifdef " << std::endl;
        dl = RDGeom::compute14DistTrans(bl1, bl2, bl3, ba12, ba23) + 0.1; //TODO 0.1 is the max that works without triangular smoothing error
        path14.type = Path14Configuration::TRANS;
        accumData.transPaths[bid1 * nb * nb + bid2 * nb + bid3] = 1;
        accumData.transPaths[bid3 * nb * nb + bid2 * nb + bid1] = 1;
// #else
...
#ifdef FORCE_TRANS_AMIDES //TODO
//std::cout<< "setmacrocycle14bound amide15 " << std::endl;
        // amide is trans, we're cis:
        dl = RDGeom::compute14DistCis(bl1, bl2, bl3, ba12, ba23);
        path14.type = Path14Configuration::CIS;
        accumData.cisPaths[bid1 * nb * nb + bid2 * nb + bid3] = 1;
...
void set14Bounds(const ROMol &mol, DistGeom::BoundsMatPtr mmat,
  ...
  const Atom *atm2 = mol.getAtomWithIdx(accumData.bondAdj->getVal(bid1, bid2));
  const Atom *atm3 = mol.getAtomWithIdx(accumData.bondAdj->getVal(bid2, bid3));
  unsigned int aid1 = mol.getBondWithIdx(bid1)->getOtherAtomIdx(atm2->getIdx());
  unsigned int aid4 = mol.getBondWithIdx(bid3)->getOtherAtomIdx(atm3->getIdx());
  //std::cout << "  Set14Bounds Function : " << aid1 << " " << aid4 << ": " <<  std::endl;

      if (rSize > 5 && rSize < 15) { //TODO ~15
